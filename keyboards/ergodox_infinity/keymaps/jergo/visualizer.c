...
static void get_visualizer_layer_and_color(visualizer_state_t* state) {
    ...
    if (state->status.layer & 0x20) {
        state->target_lcd_color = LCD_COLOR(102, saturation, 0xFF);
        ...
        state->target_lcd_color = LCD_COLOR(336, saturation, 0xFF);
        state->layer_text = "Numpad";
    }
    else if (state->status.layer & 0x4) {
        state->target_lcd_color = LCD_COLOR(252, saturation, 0xFF);
